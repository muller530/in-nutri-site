...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadProduct() {
      ...
      try {
        ...
        if (data.data) {
          // 处理tags：将JSON数组转换为逗号分隔的字符串
          let tagsString = "";
          if (data.data.tags) {
            try {
              const tagsArray = typeof data.data.tags === "string" ? JSON.parse(data.data.tags) : data.data.tags;
          ...
          setFormData({
            ...
            purchaseUrl: data.data.purchaseUrl || "",
  ...
  return (
    <div>
      <h1 className="mb-6 text-3xl font-bold text-gray-900">编辑产品</h1>
