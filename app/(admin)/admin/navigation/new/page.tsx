...
export default function NewNavigationPage() {
  ...
  useEffect(() => {
    async function loadParentMenus() {
      try {
        ...
        if (data.data) {
          // 只显示类型为 dropdown 且没有父菜单的一级菜单
          const dropdownMenus = data.data.filter(
            (item: any) => item.type === "dropdown" && !item.parentId
          );
  ...
  useEffect(() => {
    if (formData.parentId) {
      const parentMenu = parentMenus.find((m) => m.id === parseInt(formData.parentId));
      if (parentMenu) {
        // 需要获取父菜单的完整信息（包括位置）
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg bg-white p-6 shadow">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            {formData.parentId 
              ? "当前创建的是子菜单，只能选择外链或内部页面" 
              : "留空则创建一级菜单，可以选择外链、内部页面或下拉菜单"}
          </p>
        </div>
