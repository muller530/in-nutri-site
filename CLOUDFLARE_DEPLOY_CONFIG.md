# Cloudflare Pages 部署配置指南

## 📋 在 Cloudflare Pages 控制台中的配置

### 1. 基本构建设置

在 Cloudflare Pages 项目设置中，配置以下内容：

#### Framework preset（框架预设）
选择：**Next.js**

#### Build command（构建命令）
```
npm run build
```

#### Build output directory（构建输出目录）
```
.next
```

#### Root directory（根目录）
```
/（留空或填写项目根目录）
```

#### Node.js version（Node.js 版本）
```
18
```
或选择 `20`、`22`（推荐使用 18 或更高版本）

---

## 🔧 详细配置步骤

### 步骤 1: 进入 Cloudflare Pages 控制台

1. 访问 [Cloudflare Dashboard](https://dash.cloudflare.com/)
2. 点击左侧菜单的 **"Pages"**
3. 点击 **"Create a project"** 或选择现有项目

### 步骤 2: 连接 Git 仓库

1. 选择 **"Connect to Git"**
2. 选择您的 Git 提供商（GitHub、GitLab、Bitbucket）
3. 授权 Cloudflare 访问您的仓库
4. 选择要部署的仓库

### 步骤 3: 配置构建设置

在 **"Configure build"** 页面中：

#### Build settings（构建设置）

```
Framework preset: Next.js
Build command: npm run build
Build output directory: .next
Root directory: /（留空）
```

#### Environment variables（环境变量）

点击 **"Add variable"** 添加以下环境变量：

| 变量名 | 值 | 说明 |
|--------|-----|------|
| `SESSION_SECRET` | 随机字符串（至少32字符） | 用于管理员登录会话加密 |
| `NEXT_PUBLIC_BASE_URL` | `https://your-site.pages.dev` | 您的网站 URL（部署后会自动生成） |
| `R2_PUBLIC_URL` | （可选）R2 公共访问 URL | 如果配置了 R2 自定义域名 |

**生成 SESSION_SECRET 的方法：**
```bash
openssl rand -base64 32
```

### 步骤 4: 保存并部署

1. 点击 **"Save and Deploy"**
2. Cloudflare 会自动开始构建和部署

---

## 📝 完整配置示例

### 构建设置截图说明

```
┌─────────────────────────────────────┐
│ Framework preset                    │
│ [Next.js ▼]                         │
├─────────────────────────────────────┤
│ Build command                       │
│ [npm run build]                     │
├─────────────────────────────────────┤
│ Build output directory              │
│ [.next]                             │
├─────────────────────────────────────┤
│ Root directory                      │
│ [/]                                 │
├─────────────────────────────────────┤
│ Node.js version                     │
│ [18 ▼]                              │
└─────────────────────────────────────┘
```

### 环境变量配置示例

```
┌─────────────────────────────────────┐
│ Environment variables                │
├─────────────────────────────────────┤
│ SESSION_SECRET                      │
│ [your-secret-key-here...]           │
├─────────────────────────────────────┤
│ NEXT_PUBLIC_BASE_URL                │
│ [https://your-site.pages.dev]       │
└─────────────────────────────────────┘
```

---

## ⚙️ 高级配置（可选）

### 自定义构建命令

如果需要安装额外的依赖或运行迁移：

```
npm ci && npm run build
```

### 使用 Wrangler 配置

如果您使用 `wrangler.toml` 文件，确保：

1. 文件在项目根目录
2. 已正确配置 D1 数据库 ID
3. 已配置 R2 存储桶绑定

---

## 🔍 验证部署

部署成功后，检查：

1. **构建日志**：查看是否有错误
2. **网站访问**：访问生成的 `.pages.dev` 域名
3. **API 路由**：测试 `/api/products` 等路由
4. **管理员登录**：访问 `/admin/login`

---

## ⚠️ 常见问题

### 问题 1: 构建失败

**错误**：`Build failed`

**解决方案**：
- 检查 Node.js 版本（需要 18+）
- 查看构建日志中的具体错误
- 确保所有依赖都已正确安装

### 问题 2: 环境变量未生效

**错误**：环境变量在运行时未找到

**解决方案**：
- 确保环境变量名称正确
- 注意 `NEXT_PUBLIC_` 前缀（用于客户端变量）
- 重新部署以应用新的环境变量

### 问题 3: 数据库连接失败

**错误**：D1 数据库连接错误

**解决方案**：
- 检查 `wrangler.toml` 中的 `database_id`
- 确保 D1 数据库已创建
- 检查数据库绑定名称是否正确

---

## 📚 相关文档

- [Cloudflare Pages 文档](https://developers.cloudflare.com/pages/)
- [Next.js on Cloudflare Pages](https://developers.cloudflare.com/pages/framework-guides/nextjs/)
- [环境变量配置](https://developers.cloudflare.com/pages/platform/build-configuration/#environment-variables)

---

## ✅ 快速检查清单

- [ ] Framework preset 设置为 `Next.js`
- [ ] Build command 设置为 `npm run build`
- [ ] Build output directory 设置为 `.next`
- [ ] Node.js version 设置为 `18` 或更高
- [ ] `SESSION_SECRET` 环境变量已设置
- [ ] `NEXT_PUBLIC_BASE_URL` 环境变量已设置（可选，部署后会自动生成）
- [ ] Git 仓库已连接
- [ ] 代码已推送到 Git

**完成以上配置后，点击 "Save and Deploy" 即可开始部署！**






